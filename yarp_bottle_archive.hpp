...
public:
   ...
   typedef boost::mpl::bool_<false> is_saving;
	...
	void load_object(void *t, const barch::detail::basic_iserializer & bis) { throw barch::archive_exception(barch::archive_exception::unregistered_class); }
   void load_binary(void *address, std::size_t count) { throw barch::archive_exception(barch::archive_exception::unregistered_class); }
	
	template<class T>
	const barch::detail::basic_pointer_iserializer * register_type(T * = NULL) { return 0; }
   ...
   yarp_bottle_iarchive &operator>>(int &t) {
		if (! m_bottle.get(0).isInt()) throw barch::archive_exception(barch::archive_exception::unregistered_cast);
   ...
   yarp_bottle_iarchive &operator>>(double &t) {
		if (! m_bottle.get(0).isDouble()) throw barch::archive_exception(barch::archive_exception::unregistered_cast);
		t = m_bottle.get(0).asDouble();
		m_bottle = m_bottle.tail();
		return * this;
	}
   yarp_bottle_iarchive &operator>>(std::string &t) {
		if (! m_bottle.get(0).isString()) throw barch::archive_exception(barch::archive_exception::unregistered_cast);
   ...
   yarp_bottle_iarchive &operator>>(char &t) {
		if (m_bottle.get(0).isInt()) 
			...
			throw barch::archive_exception(barch::archive_exception::unregistered_cast);
		m_bottle = m_bottle.tail();
		return * this;
	}
   yarp_bottle_iarchive &operator>>(float &t) {
	...
	yarp_bottle_iarchive& operator>>(std::vector<T>& t) {
		if (! m_bottle.get(0).isList()) throw barch::archive_exception(barch::archive_exception::unregistered_cast);
		yarp::os::Bottle* b = m_bottle.get(0).asList();
		yarp_bottle_iarchive v(*b);
		t.resize(b->size());
		for (typename std::vector<T>::iterator i = t.begin(); i != t.end(); i++) v >> *i;
		m_bottle = m_bottle.tail();
		return *this;
	}
...
#else
      barch::detail::load_non_pointer_type<yarp_bottle_iarchive,T>::load_only::invoke(*this,t);
#endif
      return *this;
	}
...
public:
	...
	yarp_bottle_oarchive(yarp::os::Bottle& b) : m_bottle(b) {}
	...
	yarp_bottle_oarchive& operator<<(const std::vector<T>& t) {
		...
		for (typename std::vector<T>::const_iterator i = t.begin(); i != t.end(); i++) v << *i;
		return *this;
	}
	...
	void save_binary(const void* address, std::size_t count) { assert(false); };
	
	void save_object(const void *x, const barch::detail::basic_oserializer & bos) { assert(false);	}
...
#else
		barch::detail::save_non_pointer_type<yarp_bottle_oarchive,T>::save_only::invoke(*this,t);
#endif
		return *this;
	}

	template<class ValueType>
	void save_array(boost::serialization::array<ValueType> const& a, unsigned int) { assert(false);	}
